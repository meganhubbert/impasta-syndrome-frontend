...
const BreakfastBook = (props) => {

    let breakfastRecipes = props.user.recipes.filter(recipe => recipe.meal_type == "Breakfast")

    const showRecipeCards = () =>{
        if (props.user.recipes){
            return breakfastRecipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} deleteRecipe={props.deleteRecipe}/>)   
        }
    }
    ...
    return (
        <div>
            ...
            <div className="recipe-book-wrapper">
                <h2 className="recipe-books">what's for breakfast, {props.user.username}?</h2>
