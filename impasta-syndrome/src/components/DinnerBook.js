...
const DinnerBook = (props) => {

    let dinnerRecipes = props.user.recipes.filter(recipe => recipe.meal_type == "dinner")
    console.log(dinnerRecipes)

    const showRecipeCards = () =>{
        if (props.user.recipes){
            return dinnerRecipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} deleteRecipe={props.deleteRecipe}/>)
        }
    }
    ...
    return (
        <div>
            ...
            <div className="recipe-book-wrapper">
                <h2 className="recipe-books">what's for dinner, {props.user.username}?</h2>
