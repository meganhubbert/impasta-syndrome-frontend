...
const SnackBook = (props) => {

    let snackRecipes = props.user.recipes.filter(recipe => recipe.meal_type == "snack")

    const showRecipeCards = () =>{
        if (props.user.recipes){
            return snackRecipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} deleteRecipe={props.deleteRecipe}/>)   
        }
    }
    ...
    return (
        <div>
            ...
            <div className="recipe-book-wrapper">
                <h2 className="recipe-books">want a snack, {props.user.username}?</h2>
