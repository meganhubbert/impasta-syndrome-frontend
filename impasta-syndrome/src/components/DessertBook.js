...
const DessertBook = (props) => {

    let dessertRecipes = props.user.recipes.filter(recipe => recipe.meal_type == "dessert")

    const showRecipeCards = () =>{
        if (props.user.recipes){
            return dessertRecipes.map(recipe => <RecipeCard key={recipe.id} recipe={recipe} deleteRecipe={props.deleteRecipe}/>)   
        }
    }
    ...
    return (
        <div>
            ...
            <div className="recipe-book-wrapper">
                <h2 className="recipe-books">what's for dessert, {props.user.username}?</h2>
